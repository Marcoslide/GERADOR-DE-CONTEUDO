...
window.Views = (function () {
  ...
  function aiPanel() {
    ...
    return `
      <div class="info-block"><h4>🤖 IA Principal</h4>
        ...
        <div class="flex gap-8 center wrap" style="margin-top:6px"><span class="pill ${stCls}" id="ai-status-pill">${esc(stTxt)}</span><button class="btn btn-sm" data-act="ai-test">Testar conexão</button><button class="btn btn-sm" data-act="ai-diag">🔍 Diagnosticar conexão Claude</button><button class="btn btn-primary btn-sm" data-act="ai-save">Salvar configuração</button><button class="btn btn-sm" data-act="ai-simulated">Voltar ao simulado</button></div>
        <div id="ai-diag"></div>
        <div class="alert warn" style="margin-top:12px"><span class="al-ico">🔐</span><div class="al-body" style="font-size:12px">A chave <code>ANTHROPIC_API_KEY</code> deve ficar <b>no backend</b>. Não exponha chave no navegador. Por isso não há campo de token aqui.</div></div>
      </div>
      <div class="info-block"><h4>🧭 HTML direto x Claude Real</h4>
        <div class="alert" style="margin-bottom:10px"><span class="al-ico">💡</span><div class="al-body" style="font-size:12px"><b>HTML direto (file://) = modo simulado.</b> <b>Claude Real</b> = precisa rodar o backend com a <code>ANTHROPIC_API_KEY</code> no <code>server/.env</code>.</div></div>
      </div>
      <div class="info-block mb-0"><h4>🔌 Como testar Claude Real</h4>
        <ol style="font-size:12px;line-height:1.9;color:var(--text-1);padding-left:18px;margin:0">
          <li>Abra a pasta do projeto.</li>
          <li>Entre em <code>server/</code></li>
          <li>Copie <code>.env.example</code> para <code>.env</code></li>
          <li>Cole sua <code>ANTHROPIC_API_KEY</code> no <code>.env</code></li>
          <li>Rode <code>npm install</code></li>
          <li>Rode <code>npm start</code></li>
          <li>Abra <code>http://localhost:3000</code></li>
          <li>Selecione <b>Claude Real</b> (URL do backend pode ficar em branco)</li>
          <li>Clique em <b>Testar conexão</b> (ou <b>Diagnosticar conexão Claude</b>)</li>
        </ol>
        <p class="muted" style="font-size:11px;margin-top:8px;line-height:1.6"><code style="color:var(--accent-2)">Frontend → /api/ai/claude → Claude API</code>. Sem backend/chave, tudo cai em <b>modo simulado</b> automaticamente. Todas as gerações passam pelo <code>AIProviderService</code>.</p>
      </div>`;
  }
  ...
  function bindConfig(root) {
    ...
    root.querySelectorAll("[data-act='ai-diag']").forEach((el) => el.onclick = async () => {
      const box = root.querySelector("#ai-diag"); if (box) box.innerHTML = `<div class="muted" style="font-size:12px;margin-top:10px">Diagnosticando…</div>`;
      const d = await window.AIProvider.diagnose();
      const yn = (b, na) => na ? '<span class="pill pill-gray">n/a</span>' : (b ? '<span class="pill pill-accent">sim</span>' : '<span class="pill pill-red">não</span>');
      const rows = [
        ["Ambiente", `<code>${esc(d.runtime)}</code>`],
        ["Modo atual", esc(d.mode)],
        ["URL do backend", `<code>${esc(d.backendUrl)}</code>`],
        ["Backend respondeu?", d.onFile ? '<span class="pill pill-gray">n/a (file://)</span>' : yn(d.backendResponded)],
        ["ANTHROPIC_API_KEY detectada?", d.onFile || !d.backendResponded ? yn(false, true) : yn(d.keyDetected)],
        ["Claude respondeu?", d.keyDetected ? yn(d.claudeResponded) : yn(false, true)],
        ["JSON válido?", d.keyDetected ? yn(d.jsonValid) : yn(false, true)],
      ].map(([k, v]) => `<div class="kv-row" style="display:flex;justify-content:space-between;gap:10px;padding:4px 0;border-bottom:1px solid var(--stroke)"><span class="muted" style="font-size:12px">${k}</span><span style="font-size:12px">${v}</span></div>`).join("");
      if (box) box.innerHTML = `<div class="info-block" style="margin-top:12px;background:var(--bg-2)"><h4 style="font-size:13px">🔍 Diagnóstico</h4>${rows}<div class="alert ${d.claudeResponded ? "" : "warn"}" style="margin-top:10px"><span class="al-ico">${d.claudeResponded ? "✅" : "➡️"}</span><div class="al-body" style="font-size:12px"><b>Próximo passo:</b> ${esc(d.nextStep)}</div></div></div>`;
    });
    // Aprendizados
